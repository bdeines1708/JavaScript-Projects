//Creates an object to keep track ov values.
const Calculator = {
    //This will display 0 on the calculator screren.
    Display_Value: "0",
    ...
    //This will hold the operator, we will set itto null for now
    operator: null,
};

//This ,odifies values each time the button is clicked on.
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true) {
        ...
        //This overwrites Display_Value iff the current value is 0 otherwise it adds onto it.
...
function Input_Decimal(dot) {
    //This insures that the accidental clicking of the decimal point doesnt cause bugs in the operation
    if (Calculator.Wait_Second_Operand  === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //We care saying that of the display value does not contain a decimal point, we want to add a decimal point
...
function Handle_Operator (Next_Operator) {
    ...
    if (operator && Calculator.Wait_Second_Operand) {Calculator.operator = Next_Operator;
        return
    }
    if (First_Operand == null) {
        ...
        //if operator exists, property lookup is performed for the operator in the Perform_Calculation objectand the function that matches the operator is excexuted.
